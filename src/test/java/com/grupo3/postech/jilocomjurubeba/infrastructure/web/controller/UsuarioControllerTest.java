...
class UsuarioControllerTest {
  ...
  void deveDeletarUsuario() throws Exception {
    ...
    mockMvc.perform(delete(BASE_URL + "/10")).andExpect(status().isNoContent());
  }
  ...
  void deveAtualizarUsuario() throws Exception {
    ...
    mockMvc
        .perform(post(BASE_URL).contentType(MediaType.APPLICATION_JSON).content(criarJson))
        .andExpect(status().isCreated());
    ...
    mockMvc
        .perform(
            put(BASE_URL + "/1").contentType(MediaType.APPLICATION_JSON).content(atualizarJson))
        .andExpect(status().isOk())
