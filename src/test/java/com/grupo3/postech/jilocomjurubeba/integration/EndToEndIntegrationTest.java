 ...
 *
 * <p>
 * Simula fluxos de usuário reais:
 * - Criar restaurante com cardápio completo
 * - Atualizar múltiplos cardápios
 * - Listar restaurantes com seus cardápios
 * - Transações complexas entre entidades
 *
 * <p>
 * Foca em: Relacionamentos entre entidades, integridade de dados, fluxos
 * realistas
 */
@SpringBootTest
...
class EndToEndIntegrationTest {
    ...
    void deveExecutarFluxoCriacaoRestauranteComCardapio() throws Exception {
        ...
        mockMvc.perform(get("/restaurantes/100/cardapio")
                .contentType(MediaType.APPLICATION_JSON))
        ...
        mockMvc.perform(get("/restaurantes/100")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.nome", is("RESTAURANTE NOVO E2E")))
    ...
    void deveAtualizarMultiplosCardapios() throws Exception {
        ...
        mockMvc.perform(put("/cardapios/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(atualizar1))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(put("/cardapios/2")
                .contentType(MediaType.APPLICATION_JSON)
                .content(atualizar2))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/cardapios/2")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.preco", is(65.0)));
    }
    ...
    void deveDeletearCardapiosComRestauranteIntacto() throws Exception {
        ...
        // 2. Listar cardápios
        mockMvc.perform(get("/restaurantes/1/cardapio")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(3)));

        // 3. Deletar primeiro cardápio
        mockMvc.perform(delete("/cardapios/1")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());

        // 4. Deletar segundo cardápio
        mockMvc.perform(delete("/cardapios/2")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());
    ...
    void deveListarComFiltros() throws Exception {
        ...
        // 3. Buscar específico
        mockMvc.perform(get("/restaurantes/1/cardapio/1")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void deveManterIntegridadeDados() throws Exception {
        ...
        mockMvc.perform(get("/restaurantes/1/cardapio")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.nome == 'Novo Prato Integridade')].restauranteId", everyItem(is(1))));
    }
}
