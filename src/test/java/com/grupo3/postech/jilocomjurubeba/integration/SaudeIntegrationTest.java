 ...
 *
 * <p>Valida: - Health check endpoint disponível sem autenticação (ou com) - Retorno de status
 * correto - Formato de resposta esperado - Dados de versão e timestamp
 */
@SpringBootTest
...
class SaudeIntegrationTest {
  ...
  void deveRetornarStatus200() throws Exception {
    mockMvc.perform(get(HEALTH_URL).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
  }
  ...
  void deveRetornarStatusUp() throws Exception {
    mockMvc
        .perform(get(HEALTH_URL).accept(MediaType.APPLICATION_JSON))
  ...
  void deveRetornarComAcceptJson() throws Exception {
    mockMvc
        .perform(get(HEALTH_URL).header("Accept", "application/json"))
        .andExpect(status().isOk())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON));
  }
