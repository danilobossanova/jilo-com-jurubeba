...
class RestauranteIntegrationTest {
    ...
    void deveDeletarRestaurante() throws Exception {
        mockMvc.perform(delete(BASE_URL + "/3").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());

        // Exclusão lógica: registro continua acessível, mas inativo
        mockMvc.perform(get(BASE_URL + "/3").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
