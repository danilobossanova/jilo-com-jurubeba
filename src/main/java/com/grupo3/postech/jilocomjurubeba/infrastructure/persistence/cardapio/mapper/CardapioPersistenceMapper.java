...
/**
 * Mapper para conversao entre entidade de dominio Cardapio e JPA entity.
 *
...
public interface CardapioPersistenceMapper {
     ...
     * o Restaurante e seu dono (Usuario) a partir das referencias JPA.
     *
     * @param jpaEntity entidade JPA
     * @return entidade de dominio
     */
    default Cardapio toDomain(CardapioJpaEntity jpaEntity) {
        if (jpaEntity == null) {
            return null;
        }

        Restaurante restaurante = reconstituirRestaurante(jpaEntity.getRestaurante());
     ...
     *
     * <p>Implementacao manual. O campo restaurante da JPA entity NAO e preenchido aqui — deve ser
     * definido separadamente no Gateway para evitar problemas com proxies JPA.
     *
     * @param domain entidade de dominio
     * @return entidade JPA (sem restaurante preenchido)
     */
    default CardapioJpaEntity toJpaEntity(Cardapio domain) {
        if (domain == null) {
            return null;
        }

        CardapioJpaEntity entity = new CardapioJpaEntity();
        entity.setId(domain.getId());
        entity.setNome(domain.getNome());
        entity.setDescricao(domain.getDescricao());
        entity.setPreco(domain.getPreco());
        entity.setApenasNoLocal(domain.isApenasNoLocal());
        entity.setCaminhoFoto(domain.getCaminhoFoto());
        entity.setAtivo(domain.isAtivo());
        // restaurante e definido separadamente no gateway
        return entity;
    }
     ...
     */
    default List<Cardapio> toDomainList(List<CardapioJpaEntity> jpaEntities) {
        if (jpaEntities == null) {
    ...
    /**
     * Reconstitui a entidade de dominio Restaurante a partir da JPA entity.
     *
     * @param restauranteJpa entidade JPA do restaurante
     * @return entidade de dominio Restaurante ou null
     */
    private Restaurante reconstituirRestaurante(RestauranteJpaEntity restauranteJpa) {
        if (restauranteJpa == null) {
            return null;
        }

        Usuario dono = reconstituirDono(restauranteJpa.getDono());

        return new Restaurante(
                restauranteJpa.getId(),
                restauranteJpa.getNome(),
                restauranteJpa.getEndereco(),
                restauranteJpa.getTipoCozinha(),
                restauranteJpa.getHoraAbertura(),
                restauranteJpa.getHoraFechamento(),
                dono,
                restauranteJpa.isAtivo());
    }
