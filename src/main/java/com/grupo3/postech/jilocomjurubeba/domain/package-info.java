 ...
 *
 * Esta camada contém:
 * - Entidades de domínio (regras de negócio encapsuladas)
 * - Value Objects (objetos imutáveis que representam conceitos do domínio)
 * - Interfaces de Gateway (portas para o mundo externo)
 * - Exceções de domínio
 * - Enums de domínio
 *
 * REGRAS IMPORTANTES:
 * 1. NÃO pode depender de frameworks (Spring, JPA, etc.)
 * 2. NÃO pode depender de outras camadas (application, infrastructure, interfaces)
 * 3. Deve ser 100% testável sem mocks de framework
 * 4. Entidades devem proteger seus invariantes
 *
